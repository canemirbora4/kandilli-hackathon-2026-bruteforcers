...
import ExpertSidebar, { buildExportJSON, type DigitizeResult } from "@/components/ExpertSidebar";
...
export default function AdminPage() {
  ...
  const [digitizeResult, setDigitizeResult] = useState<DigitizeResult | null>(null);
  ...
  const handleDigitize = useCallback(async () => {
    ...
    const boxes = annotation.boundingBoxes.map((b) => ({
      BoxCoord: [b.xmin, b.ymin, b.xmax, b.ymax],
      BoxType: b.boxType || "Siliklik",
      Boxenter: b.box_enter ? [b.box_enter.x, b.box_enter.y] : null,
    ...
    const payload = {
      ...
      timestamp: selectedFile.date
        ? `${selectedFile.year}-${String(months.find(m => m.name === selectedMonth)?.num || 1).padStart(2, "0")}-${selectedFile.date}`
        : undefined,
      start_point: [annotation.startPoint.x, annotation.startPoint.y],
  ...
  const overlayImageSrc = digitizeResult?.overlay_base64
    ? `data:image/jpeg;base64,${digitizeResult.overlay_base64}`
    : null;
